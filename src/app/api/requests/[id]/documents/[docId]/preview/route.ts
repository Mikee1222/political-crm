...
export async function GET(_request: NextRequest, context: PreviewParams) {
  try {
    ...
    const admin = createServiceClient();
    const selectCols = "id, request_id, name, file_url, file_type";
    ...
    if (doc) {
      console.info("[documents preview] found with ownership", { docId, requestId });
    } else {
      console.warn("[documents preview] ownership miss, trying id-only", { docId, requestId });
      ...
      if (!doc) {
        console.warn("[documents preview] not found", { docId, requestId });
    ...
    console.error("[api/requests/documents/preview]", e);
