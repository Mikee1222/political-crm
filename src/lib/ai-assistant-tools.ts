...
export function mapSpreadsheetRowToContactPayload(
  ...
  return transformSpreadsheetRow(row, mapping, options);
}
...
async function runAlexToolInner(
  ...
  if (name === "smart_excel_import") {
    ...
    const { work, validation, sample } = buildImportPreview(rows, mapObj, {
      contextMunicipality: contextMunicipality || undefined,
    });
    ...
    for (let b = 0; b < phones.length; b += 120) {
      ...
      const { data: phRows } = await ctx.supabase.from("contacts").select("id, phone, first_name, last_name").in("phone", part);
    ...
    if (!confirmed) {
      ...
      const sampleLines = sample
        .slice(0, 3)
        .map((s) => `${s.first_name} ${s.last_name} — ${s.phone || "χωρίς τηλ."}${s.municipality ? ` — ${s.municipality}` : ""}`)
        .join("\n");
      const dupHint =
        dups.length > 0
      ...
      return {
        content: JSON.stringify({
          ...
          summary: {
            ...
            invalid_phones: validation.invalid_phones,
            duplicates: dups.length,
            missing_names: validation.missing_names,
            skip_no_phone: validation.skip_no_phone,
            skip_incomplete_name: validation.skip_incomplete_name,
          },
          detected_columns: mapObj,
          sample,
          duplicates: dups.slice(0, 5),
          pending_rows: work.filter((w) => w.payload).map((w) => w.payload),
          message: `📊 Προεπισκόπηση εισαγωγής:
- Σύνολο γραμμών: ${validation.total}
- Έγκυρα τηλέφωνα: ${validation.valid_phones}
- Μη έγκυρα τηλέφωνα: ${validation.invalid_phones}
- Διπλότυπα: ${dups.length}
- Χωρίς όνομα: ${validation.missing_names}
- Χωρίς τηλέφωνο: ${validation.skip_no_phone}
...
Επιβεβαιώνεις την εισαγωγή; (ναι/όχι)`,
  ...
  if (name === "export_contacts") {
    ...
    try {
      ...
      const stored = await buildAlexandraExportMatrix(ctx.userId, headers, rows, filename, format, "Επαφές");
      return {
        content: JSON.stringify({
          ok: true,
