...
export const lux = {
  ...
  input:
    "hq-form-control h-[42px] w-full min-w-0 max-w-full rounded-lg border border-[var(--border)] bg-[var(--input-bg)] px-3 text-sm text-[var(--text-input)] placeholder:text-[var(--text-placeholder)] transition-[border-color,box-shadow] duration-150 ease-in-out focus:border-[var(--accent-gold)] focus:outline focus:outline-2 focus:outline-[var(--accent-gold)] focus:outline-offset-2 focus:ring-0 disabled:cursor-not-allowed disabled:opacity-50 max-md:min-h-[44px] max-md:text-base",
  ...
  btnIcon:
    "inline-flex h-11 min-h-[44px] w-11 min-w-[44px] items-center justify-center rounded-lg border border-[var(--border)] bg-[var(--bg-elevated)] text-[var(--text-secondary)] transition duration-150 hover:border-[var(--border-hover)] hover:bg-[var(--bg-card)] hover:text-[var(--text-primary)] active:scale-[0.98]",
