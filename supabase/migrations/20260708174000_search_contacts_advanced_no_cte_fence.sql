-- Avoid dual-referencing a filtered CTE (page + count), which forces
-- materializing ~24k rows before LIMIT. Page query is a single SELECT;
-- group-only totals use count_contacts_in_groups_fast (or API parallel).
...
as $$
  with params as (
    select
      ...
      coalesce(p_compute_total, true) as compute_total
  ),
  flags as (
    select
      p.*,
  ...
  -- Scalar total only — never shared with the page rowset (avoids CTE materialization).
  total as (
    select case
      when not (select compute_total from flags) then 0::bigint
      when (select is_group_only from flags) then
        public.count_contacts_in_groups_fast(
          (select include_ids from flags),
          (select match_mode from flags)
        )
      else (
        select count(*)::bigint
        from contacts c
            ...
            or (
              ...
              and (
                exists (
                  ...
                  where cgm.contact_id = c.id and cgm.group_id = any (p.include_ids)
                )
  ...
  select
    c.id, c.first_name, c.last_name, c.father_name, c.phone, c.phone2, c.landline, c.email,
    c.area, c.municipality, c.toponym, c.gender, c.call_status, c.priority, c.tags, c.nickname,
    c.contact_code, c.age, c.political_stance, c.group_id, c.birthday,
    extract(year from c.birthday)::int as birth_year, c.predicted_score::numeric, c.is_volunteer,
    c.volunteer_role, c.volunteer_area, c.volunteer_since, c.language, c.last_contacted_at,
    c.name_day::text, c.is_dead, c.electoral_district, c.may_not_have_mobile,
    c.may_not_have_landline, c.may_not_have_email, c.created_at,
  ...
  order by c.last_name asc nulls last, c.first_name asc nulls last, c.id asc
