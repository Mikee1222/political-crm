-- Fast total for group-primary search_contacts_advanced:
-- 1) Drop COUNT(*) OVER() from the page query (scan only the requested page)
-- 2) Group-only OR/AND totals via index-friendly membership counts
-- 3) Filtered combos keep a correct separate COUNT(*) subquery
...
as $$
  ...
  membership as (
    ...
    cross join params p
    where p.group_ids is not null
      and cardinality(p.group_ids) > 0
      and cgm.group_id = any (p.group_ids)
    union
    ...
    where p.group_ids is not null
      and cardinality(p.group_ids) > 0
      and c.group_id = any (p.group_ids)
  )
  select coalesce(
    case
      ...
      when (select match_mode from params) = 'AND' then (
        ...
        from (
          ...
          having count(distinct m.group_id) = (
            select cardinality(group_ids) from params
          )
        ) sub
      )
      else (
        select count(distinct m.contact_id)::bigint from membership m
      )
    end,
    0::bigint
  );
...
drop function if exists public.search_contacts_advanced(
